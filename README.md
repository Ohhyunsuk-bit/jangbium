...
GitHub Pages, `gh-pages` 브랜치. 공개 주소: https://ohhyunsuk-bit.github.io/jangbium/
...
- 페이지의 canonical·og:image·sitemap은 현재 위 GitHub Pages 주소로 고정되어 있다. 나중에 도메인(`jangbium.com` 등)을 붙이면 Settings → Pages → Custom domain에 등록하고, `*.html`·`sitemap.xml`·`robots.txt`의 `https://ohhyunsuk-bit.github.io/jangbium`을 새 주소로 일괄 치환한 뒤 다시 배포한다.
