...
```js
import { FLAG_CHARS, SSL_BANDS, computeRecall, buildRecallEvents, buildReportText } from '../js/recall.js';
...
export function buildReportText(result, examDate) {
  ...
  const when =
    result.months[0] === result.months[1] ? `${formatMonths(result.months[0])} 후`
    : `${formatMonths(result.months[0])}~${formatMonths(result.months[1])} 후`;
  const whenDate =
...
- Consumes: Task 1의 `esc, toast, copyText`(`ui-utils.js`), Task 2-4의
  `computeRecall, buildReportText, formatYearMonthKo, formatMonths, parseDateOnly,
...
```js
import {
  computeRecall, buildReportText, formatYearMonthKo, formatMonths,
...
function formatRange(result) {
  const big =
    result.months[0] === result.months[1] ? `${formatMonths(result.months[0])} 뒤`
    : `${formatMonths(result.months[0])}~${formatMonths(result.months[1])} 뒤`;
  const dateLine =
    result.dates.length > 1 ? `${formatYearMonthKo(result.dates[0])} ~ ${formatYearMonthKo(result.dates[1])}`
...
파일 맨 위 import에 필요한 것을 더한다:

```js
import {
  computeRecall, buildReportText, buildRecallEvents, formatYearMonthKo, formatMonths,
